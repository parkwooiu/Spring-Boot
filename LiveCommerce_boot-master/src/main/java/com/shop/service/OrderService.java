...
//        Item item = results.orElseThrow(() -> new EntityNotFoundException());
    ...
    public OrderDetailDto getOrderDetail(Long orderId) {
        ...
        // 주문에 포함된 첫 번째 상품의 정보만 가져옴 (단일 상품 구매를 가정)
        OrderItem firstOrderItem = orderItems.stream().findFirst()
                .orElseThrow(EntityNotFoundException::new);

        Item item = firstOrderItem.getItem();
        String itemName = item.getItemNm();
        int orderPrice = firstOrderItem.getOrderPrice();
        int count = firstOrderItem.getCount();
        String memberName = member.getName();
        String memberEmail = member.getEmail();
        String itemDetail = item.getItemDetail();
        int itemStockNumber = item.getStockNumber();

        OrderDetailDto orderDetailDto = new OrderDetailDto();
        orderDetailDto.setOrderId(orderId);
        orderDetailDto.setItemId(item.getId());
        orderDetailDto.setItemName(itemName);
        orderDetailDto.setOrderPrice(orderPrice);
        orderDetailDto.setCount(count);
        orderDetailDto.setMemberName(memberName);
        orderDetailDto.setMemberEmail(memberEmail);
        orderDetailDto.setItemDetail(itemDetail);
        orderDetailDto.setItemStockNumber(itemStockNumber);
